...
export const globalContextWrapper = (WrappedComponent) => {
  return class GlobalContextWrapper extends React.Component {
        ...
        db.collection("user_categories")
          ...
          .then((uCatsSnapshot) => {
            uCatsSnapshot.forEach((doc) => {
              ...
              userSelectedCategories.push(
                categories.find((c) => c.value === data.category_id)
              );
            });
    ...
    onSelectionChange = (userSelectedCategories, selectedCategory) => {
      if (
        userSelectedCategories.length > this.state.userSelectedCategories.length
      ) {
        db.collection("user_categories")
          .add({
    ...
    setShouldRefreshEvents = (shouldRefreshEvents) =>
      this.setState({ shouldRefreshEvents });

    render() {
      const { categories, userSelectedCategories, shouldRefreshEvents } =
        this.state;
      return (
